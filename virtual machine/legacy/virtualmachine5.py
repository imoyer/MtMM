...
class controller(object):
    ...
    serport = serial.Serial('COM2',baudrate, timeout=None)
    ...
    def stepgen (self, traverse, rate):             #note: rate in in/min
        ...
        neededtimeperpulse = time / duration
...
#        serport = serial.Serial(portnumber,baudrate, timeout=sertimeout)
        ...
        #-------SEND COMMAND-------------------------------------------------
        outgoing = [255,6,0,0,0,0,50,0,196,9,0,0,16]
        for i in range(len(outgoing)):
            machinecontroller.serport.write(chr(outgoing[i]))
            a = machinecontroller.serport.read()
            print ord(a)
    ...
    def simmove(self, outgoing):
        ...
        time =   maxprescalar * maxhardwarecounter * duration / minclockspeed

        minutes = time / 60 #time in minutes

        rate = distance / minutes

        print distance
        return [delta, rate, time]
...
print delta
machinecontroller.stepgen(delta, 4)
outgoing = machinecontroller.xmit()
[realdelta, realrate, realtime] = machinecontroller.simmove(outgoing)

#portnumber = 3
#baudrate = 19200
#sertimeout = None #in seconds
#serport = serial.Serial(portnumber,baudrate, timeout=sertimeout)
b = machinecontroller.serport.read()
print ord(b)

#fudgefactor = 50/44.8
#fudgedtime=time*fudgefactor
