...
class controller(object):
    ...
    def stepgen (self, traverse, rate):             #note: rate in in/min
        ...
        maxsteps = max(movingsteps)
        minsteps = min(movingsteps)

        stepproduct = minsteps*maxsteps

    
        stepinterval = stepproduct / movingsteps
        print stepinterval
        ...
        minrangediff = min(stepintervalrange)
        dynamicrangeresolution = virtualmachine.dynamicrangeresolution



        
        stepratemultiplier = min(dynamicrangeresolution/minrangediff, dynamicrangeresolution)
        

        durations = stepinterval*stepratemultiplier
...
moveto = numpy.array([5.999,0.001,0])
