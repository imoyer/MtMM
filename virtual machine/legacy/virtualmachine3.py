...
class controller(object):
    ...
    def stepgen (self, traverse, rate):             #note: rate in in/min
        ...
        stepinterval = numpy.ones(len(movingsteps))/movingsteps        #this converts moving steps into time between steps


        orderedindex = numpy.argsort(stepinterval)

        movingaxes = numpy.take(stepinterval,orderedindex)     #this section places all step arrays into an order from min to max step interval
        movingsteps = numpy.take(movingsteps, orderedindex)
        stepinterval = numpy.take(stepinterval, orderedindex)
        minstepinterval = min(stepinterval)
        stepinterval = stepinterval / minstepinterval           #normalizes step interval matrix (fastest step rate is 1)

        print stepinterval


 
        stepindices = numpy.array(range(len(stepinterval)))            #here the step interval matrix gets shifted (logical rotate) by 1
        stepindicesrot = stepindices + 1
        stepindicesrot[len(stepindices)-1]= 0
        stepintervalrot = numpy.take(stepinterval,stepindicesrot)
        print stepintervalrot

        stepintervalrange = stepintervalrot - stepinterval              #determines the minimum difference in rate between axes
        stepintervalrange = stepintervalrange[0:(len(stepintervalrange)-1)]
        

        roundedintervalrange = stepintervalrange.astype(int)
        rangediff = stepintervalrange-roundedintervalrange
        print rangediff
        rangediffindex = numpy.nonzero(rangediff)[0]                #removes all zeros (i.e. identically moving axes)
        rangediff = numpy.take(rangediff, rangediffindex)

        minrangediff = min(rangediff)
        differentialdynamicrange = math.ceil(1 / minrangediff)

        durations = stepinterval*differentialdynamicrange
        
        
        print rangediff
        print minrangediff
        print differentialdynamicrange
        print stepinterval
